#pragma once

#include <vector>
#include <thread>
#include <atomic>
#include <functional>
#include <memory>
#include "NetTCPSocket.h"
#include "NetDef.h"
#include "IPacket.h"


namespace Net {

    class TCPClientSocket {
    public:
        TCPClientSocket();
        ~TCPClientSocket();

        // Start: 소켓 생성, 서버 연결 및 수신 스레드 시작
        bool Start(const NetAddress& serverAddress);

        // Stop: 연결 종료 및 수신 스레드 정리
        void Stop();

        // 패킷 수신 콜백 (수신된 IPacket 객체를 전달)
        std::function<void(std::unique_ptr<IPacket> packet)> OnPacketReceived;

        // IPacket 전송 (패킷 형식: [4바이트 길이][PayloadHeader][payload])
        bool SendPacket(const IPacket& packet);

    private:
        NetTCPSocket socket_;
        std::vector<char> recvBuffer_; // 누적 수신 데이터 버퍼
        int bufferSize_;
        std::atomic<bool> running_;
        std::thread recvThread_;

        // 내부 함수: 수신 스레드 루프
        void RecvThread();

        // 내부 함수: 누적 버퍼에서 완전한 패킷 추출 ([4바이트 길이][PayloadHeader][payload])
        bool ProcessBuffer(std::vector<char>& packetData);
    };
} // namespace Net
