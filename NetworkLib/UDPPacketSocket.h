...
#pragma pack(pop)
    ...
    public:
        ...
        // 수신된 패킷 처리 후 반환
        std::pair<bool, bool> RecvPacket(RecvPacketInfo& packetInfo);

        // 지금까지 수신된 패킷 큐에 저장 ( RecvPacket 래퍼 )
        void ReceivePacketsToQueue();

        // 큐에 저장된 패킷을 콜백처리
        void ProcessPacketsFromQueue(const std::function<void(RecvPacketInfo&)>& callback);
    ...
    private:
        ...
        ConcurrentQueue<RecvPacketInfo> recv_data_queue_;
