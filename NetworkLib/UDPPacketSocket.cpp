...
namespace Net::UDP {
    ...
    std::pair<bool, bool> UDPPacketSocket::RecvPacket(ReceivedPacketInfo& packetInfo)
    {
    ...
    void UDPPacketSocket::ProcessPacketsFromQueue(const std::function<void(ReceivedPacketInfo&)>& callback)
    {
        if (!callback) return;
