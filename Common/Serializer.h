...
public:
	...
	static void Serialize(BYTE* pos, std::string* arr,uint8_t len, int& currentByte);
	...
	static int GetPacketSize(std::string* arr,uint8_t len);
	...
	static uint8_t GetArrLen(std::string* arr);
	static uint8_t GetArrLen(void* arr,uint8_t dataSize);
};
