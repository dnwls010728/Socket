...
GeneralAttackSkill::GeneralAttackSkill(PlayerCharacter* owner,  const SkillData* data, int32_t level)
: ActiveSkill(owner, data, level), next_frame_(0), elapsed_(0.f), is_flipped_(false), owner_position_(Math::Vector2::Zero())
{
}

void GeneralAttackSkill::OnStart()
{
    next_frame_ = 0;
    elapsed_ = 0.f;
...
void GeneralAttackSkill::Tick(float delta_time)
{
    if (!IsStart()) return;
...
void GeneralAttackSkill::ApplyHitFrame(const HitFrame& frame)
{
    if (owner_ == nullptr) return;
