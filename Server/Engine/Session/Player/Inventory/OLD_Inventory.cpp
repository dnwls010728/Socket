...
OLD_Inventory::OLD_Inventory(PlayerCharacter* owner) :
    owner_(owner),
    inventories_(),
    slot_capacity_()
{
...
uint32_t OLD_Inventory::FindFreeSlot(Type type)
{
    std::lock_guard<std::mutex> lock(mutex_);
    uint32_t counter = 1;
...
std::shared_ptr<Item> OLD_Inventory::FindItem(Type type, uint32_t slot_id)
{
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = inventories_[static_cast<uint8_t>(type)].find(slot_id);
    if (it != inventories_[static_cast<uint8_t>(type)].end())
...
void OLD_Inventory::AddSlot(Type type, uint32_t slot_id, uint32_t item_id, int32_t count)
{
...
void OLD_Inventory::RemoveItem(Type type, uint32_t slot_id, int32_t count)
{
...
void OLD_Inventory::GetItems(std::vector<std::shared_ptr<Item>>& out_items, uint32_t item_id) const
{
...
bool OLD_Inventory::AddItem(const std::shared_ptr<Item>& item)
{
...
bool OLD_Inventory::UpdateDatabase() const
{
...
void OLD_Inventory::Remove_Internal(Type type, uint32_t slot_id)
{
    auto it = inventories_[static_cast<uint8_t>(type)].find(slot_id);
