...
void Serializer::Serialize(BYTE* pos, string value, int& currentByte)
{
    pos[currentByte] = 7;
    currentByte++;
...
int Serializer::GetPacketSize(string value)
{
    // 구분자 1바이트, 문자열 길이 1바이트, 널문자 1바이트
    return strlen(value.c_str()) + 3;
}

int Serializer::GetPacketSize(uint16_t* arr, uint8_t len)
{
    //구분자 1바이트, 배열 길이 1바이트, 배열 길이 * 자료형 크기
