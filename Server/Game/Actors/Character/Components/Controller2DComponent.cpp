...
void Controller2DComponent::HorizontalCollisions(Math::Vector2& move_amount)
{
    ...
    for (int32_t i = 0; i < horizontal_ray_count_; ++i)
    {
        ...
        bool is_hit = Physics2D::RayCast(map, hit_result, ray_origin, Math::Vector2::Right() * direction_x, ray_length, static_cast<uint16_t>(ActorLayer::kDefault));
...
void Controller2DComponent::VerticalCollisions(Math::Vector2& move_amount)
{
    ...
    for (int32_t i = 0; i < vertical_ray_count_; ++i)
    {
        ...
        Map& map = *owner_->GetMap();
        HitResult hit_result;
        bool is_hit = Physics2D::RayCast(map, hit_result, ray_origin, Math::Vector2::Up() * direction_y, ray_length, static_cast<uint16_t>(ActorLayer::kDefault));
...
void Controller2DComponent::DescendSlope(Math::Vector2& move_amount)
{
    ...
    Map& map = *owner_->GetMap();
    HitResult hit_result;
    bool is_hit = Physics2D::RayCast(map, hit_result, ray_origin, Math::Vector2::Down(), std::numeric_limits<float>::max(), static_cast<uint16_t>(ActorLayer::kDefault));
