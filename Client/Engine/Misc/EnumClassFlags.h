...
#define ENUM_CLASS_FLAGS(enum_class) \
    ...
    \
    FORCEINLINE constexpr enum_class operator~(enum_class rhs) \
    { \
        using T = std::underlying_type_t<enum_class>; \
        return static_cast<enum_class>(~static_cast<T>(rhs)); \
    } \
    \
    FORCEINLINE enum_class& operator|=(enum_class& lhs, enum_class rhs) \
    { \
        using T = std::underlying_type_t<enum_class>; \
    ...
    \
    FORCEINLINE bool operator!(enum_class rhs) \
    { \
        using T = std::underlying_type_t<enum_class>; \
