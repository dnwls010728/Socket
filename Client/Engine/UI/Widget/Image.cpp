...
void UI::Image::SetTexture(const std::wstring& path)
{
    texture_ = AssetManager::Get()->Load<UITexture>(path);
}

std::shared_ptr<UI::Image> UI::Image::Create(const std::wstring& name)
{
    return std::make_shared<Image>(name);
}

void UI::Image::Render(Renderer* renderer, WindowsWindow* window)
{
...
RTTR_REGISTRATION
{
    ...
    registration::class_<UI::Image>("UI::Image")
