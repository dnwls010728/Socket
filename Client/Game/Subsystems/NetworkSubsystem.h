...
public:
    ...
    std::shared_ptr<T> SpawnNetworkActor(const rttr::type& type, Type::uint32 unique_id, const std::wstring& name = L"");
...
template <std::derived_from<NetworkActor> T>
std::shared_ptr<T> NetworkSubsystem::SpawnNetworkActor(const rttr::type& type, Type::uint32 unique_id, const std::wstring& name)
{
